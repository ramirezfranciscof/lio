...
//          hessian_values(2 * (ii + 2) + 1, point) = vec_type3((vxxy * vyzz) * 4 * th * v.z() - vec_type3(0, v.x(), v.y()) * 2 * tg);
        ...
        if (forces || gga) {
          ...
          gX(ii, point) = (2 * v.x() * t - v.x() * 2 * tg * v.x() * v.x()) * fortran_vars.normalization_factor;
          gY(ii, point) = (0 * t - v.y() * 2 * tg * v.x() * v.x()) * fortran_vars.normalization_factor;
          gZ(ii, point) = (0 * t - v.z() * 2 * tg * v.x() * v.x()) * fortran_vars.normalization_factor;
          ...
          gX(ii+2, point) = (0 * t - v.x() * 2 * tg * v.y() * v.y()) * fortran_vars.normalization_factor;
          gY(ii+2, point) = (2 * v.y() * t - v.y() * 2 * tg * v.y() * v.y()) * fortran_vars.normalization_factor;
          gZ(ii+2, point) = (0 * t - v.z() * 2 * tg * v.y() * v.y()) * fortran_vars.normalization_factor;
          ...
          gX(ii+5, point) = (0 * t - v.x() * 2 * tg * v.z() * v.z()) * fortran_vars.normalization_factor;
          gY(ii+5, point) = (0 * t - v.y() * 2 * tg * v.z() * v.z()) * fortran_vars.normalization_factor;
          gZ(ii+5, point) = (2 * v.z() * t - v.z() * 2 * tg * v.z() * v.z()) * fortran_vars.normalization_factor;
        }
...
//          hessian_values(2 * (ii + 0) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.x() * v.x()) - vec_type3(10, 2, 2) * tg * (v.x() * v.x())    + vec_type3(2 * t, 0    , 0)) * fortran_vars.normalization_factor);
          hPX(ii, point) = (v.x() * v.x() * 4 * th * v.x() * v.x() - 10 * tg * v.x() * v.x() + 2* t) * fortran_vars.normalization_factor;
          hPY(ii, point) = (v.y() * v.y() * 4 * th * v.x() * v.x() - 2 * tg * v.x() * v.x() + 0) * fortran_vars.normalization_factor;
          hPZ(ii, point) = (v.z() * v.z() * 4 * th * v.x() * v.x() - 2 * tg * v.x() * v.x() + 0) * fortran_vars.normalization_factor;
//          hessian_values(2 * (ii + 0) + 1, point) = vec_type3(((vxxy * vyzz) * 4 * th * (v.x() * v.x()) - vec_type3(4,  4, 0) * tg * (vxxy * vyzz)                                 ) * fortran_vars.normalization_factor);
          hIX(ii, point) = (vxxy.x() * vyzz.x() * 4 * th * v.x() * v.x() - 4 * tg * vxxy.x() * vyzz.x()) * fortran_vars.normalization_factor;
          hIY(ii, point) = (vxxy.y() * vyzz.y() * 4 * th * v.x() * v.x() - 4 * tg * vxxy.y() * vyzz.y()) * fortran_vars.normalization_factor;
          hIZ(ii, point) = (vxxy.z() * vyzz.z() * 4 * th * v.x() * v.x() - 0 * tg * vxxy.z() * vyzz.z()) * fortran_vars.normalization_factor;
//          hessian_values(2 * (ii + 1) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.x() * v.y()) - vec_type3(6,  6, 2) * tg * (v.x() * v.y())                                   ));
          hPX(ii+1, point) = v.x() * v.x() * 4 * th * v.x() * v.y() - 6 * tg * v.x() * v.y();
          hPY(ii+1, point) = v.y() * v.y() * 4 * th * v.x() * v.y() - 6 * tg * v.x() * v.y();
          hPZ(ii+1, point) = v.z() * v.z() * 4 * th * v.x() * v.y() - 2 * tg * v.x() * v.y();
//          hessian_values(2 * (ii + 1) + 1, point) = vec_type3(((vxxy * vyzz) * 4 * th * (v.x() * v.y()) - vec_type3(2 * (v.x() * v.x() + v.y() * v.y()), 2 * v.y() * v.z(), 2 * v.x() * v.z()) * tg + vec_type3(t     , 0    , 0)));
          hIX(ii+1, point) = vxxy.x() * vyzz.x() * 4 * th * v.x() * v.y() - 2 * (v.x() * v.x() + v.y() * v.y()) * tg + t;
          hIY(ii+1, point) = vxxy.y() * vyzz.y() * 4 * th * v.x() * v.y() - 2 * v.y() * v.z() * tg + 0;
          hIZ(ii+1, point) = vxxy.z() * vyzz.z() * 4 * th * v.x() * v.y() - 2 * v.x() * v.z() * tg + 0;
//          hessian_values(2 * (ii + 2) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.y() * v.y()) - vec_type3(2, 10, 2) * tg * (v.y() * v.y()) + vec_type3(0    , 2 * t, 0)) * fortran_vars.normalization_factor);
          hPX(ii+2, point) = (v.x() * v.x() * 4 * th * v.y() * v.y() - 2 * tg * v.y() * v.y() + 0) * fortran_vars.normalization_factor;
          hPY(ii+2, point) = (v.y() * v.y() * 4 * th * v.y() * v.y() - 10 * tg * v.y() * v.y() + 2 * t) * fortran_vars.normalization_factor;
          hPZ(ii+2, point) = (v.z() * v.z() * 4 * th * v.y() * v.y() - 2 * tg * v.y() * v.y() + 0) * fortran_vars.normalization_factor;
//          hessian_values(2 * (ii + 2) + 1, point) = vec_type3(((vxxy * vyzz) * 4 * th * (v.y() * v.y()) - vec_type3(4,  0, 4) * tg * (vxxy * vyzz)                                ) * fortran_vars.normalization_factor);
          hIX(ii+2, point) = (vxxy.x() * vyzz.x() * 4 * th * (v.y() * v.y()) - 4 * tg * vxxy.x() * vyzz.x()) * fortran_vars.normalization_factor;
          hIY(ii+2, point) = (vxxy.y() * vyzz.y() * 4 * th * (v.y() * v.y()) - 0 * tg * vxxy.y() * vyzz.y()) * fortran_vars.normalization_factor;
          hIZ(ii+2, point) = (vxxy.z() * vyzz.z() * 4 * th * (v.y() * v.y()) - 4 * tg * vxxy.z() * vyzz.z()) * fortran_vars.normalization_factor;
//          hessian_values(2 * (ii + 3) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.x() * v.z()) - vec_type3(6,  2, 6) * tg * (v.x() * v.z())                                  ));
          hPX(ii+3, point) = v.x() * v.x() * 4 * th * v.x() * v.z() - 6 * tg  * v.x() * v.z();
          hPY(ii+3, point) = v.y() * v.y() * 4 * th * v.x() * v.z() - 2 * tg  * v.x() * v.z();
          hPZ(ii+3, point) = v.z() * v.z() * 4 * th * v.x() * v.z() - 6 * tg  * v.x() * v.z();
//          hessian_values(2 * (ii + 3) + 1, point) = vec_type3(((vxxy * vyzz) * 4 * th * (v.x() * v.z()) - vec_type3(2 * v.y() * v.z(), 2 * (v.x() * v.x() + v.z() * v.z()), 2 * v.x() * v.y()) * tg + vec_type3(0,      t,     0)));
          hIX(ii+3, point) = vxxy.x() * vyzz.x() * 4 * th * v.x() * v.z() - 2 * v.y() * v.z() * tg + 0;
          hIY(ii+3, point) = vxxy.y() * vyzz.y() * 4 * th * v.x() * v.z() - 2 * (v.x() * v.x() + v.z() * v.z()) * tg + t;
          hIZ(ii+3, point) = vxxy.z() * vyzz.z() * 4 * th * v.x() * v.z() - 2 * v.x() * v.y() * tg + 0;
//           hessian_values(2 * (ii + 4) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.y() * v.z()) - vec_type3(2,  6, 6) * tg * (v.y() * v.z())                                ));
          hPX(ii+4, point) = v.x() * v.x() * 4 * th * v.y() * v.z() - 2 * tg * v.y() * v.z();
          hPY(ii+4, point) = v.y() * v.y() * 4 * th * v.y() * v.z() - 6 * tg * v.y() * v.z();
          hPZ(ii+4, point) = v.z() * v.z() * 4 * th * v.y() * v.z() - 6 * tg * v.y() * v.z();
//          hessian_values(2 * (ii + 4) + 1, point) = vec_type3(((vxxy * vyzz) * 4 * th * (v.y() * v.z()) - vec_type3(2 * v.x() * v.z(), 2 * v.x() * v.y(), 2 * (v.y() * v.y() + v.z() * v.z())) * tg + vec_type3(0,      0,     t)));
          hIX(ii+4, point) = vxxy.x() * vyzz.x() * 4 * th * v.y() * v.z() - 2 * v.x() * v.z() * tg + 0;
          hIY(ii+4, point) = vxxy.y() * vyzz.y() * 4 * th * v.y() * v.z() - 2 * v.x() * v.y() * tg + 0;
          hIZ(ii+4, point) = vxxy.z() * vyzz.z() * 4 * th * v.y() * v.z() - 2 * (v.y() * v.y() + v.z() * v.z()) * tg + t;
//          hessian_values(2 * (ii + 5) + 0, point) = vec_type3(((v * v)       * 4 * th * (v.z() * v.z()) - vec_type3(2,  2, 10) * tg * (v.z() * v.z()) + vec_type3(0,      0, 2 * t)) * fortran_vars.normalization_factor);
          hPX(ii+5, point) =  (v.x() * v.x() * 4 * th * v.z() * v.z() - 2 * tg * v.z() * v.z() + 0) * fortran_vars.normalization_factor;
          hPY(ii+5, point) =  (v.y() * v.y() * 4 * th * v.z() * v.z() - 2 * tg * v.z() * v.z() + 0) * fortran_vars.normalization_factor;
          hPZ(ii+5, point) =  (v.z() * v.z() * 4 * th * v.z() * v.z() - 10 * tg * v.z() * v.z() + 2 * t) * fortran_vars.normalization_factor;
          ...
          hIX(ii+5, point) =  (vxxy.x() * vyzz.x() * 4 * th * v.z() * v.z() - 0 * tg * vxxy.x() * vyzz.x()) * fortran_vars.normalization_factor;
          hIY(ii+5, point) =  (vxxy.y() * vyzz.y() * 4 * th * v.z() * v.z() - 4 * tg * vxxy.y() * vyzz.y()) * fortran_vars.normalization_factor;
          hIZ(ii+5, point) =  (vxxy.z() * vyzz.z() * 4 * th * v.z() * v.z() - 4 * tg * vxxy.z() * vyzz.z()) * fortran_vars.normalization_factor;
        }
        ii += 6;
      }
    }
