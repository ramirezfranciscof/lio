  ...
  scalar_type* factor_a_sh, scalar_type* factor_c_sh, uint nuc, scalar_type& t, scalar_type& tg, scalar_type& th, vec_type<scalar_type,3>& v, int current_device)
{
  ...
  if (do_gga) th = 0.0f;
	...
	for (uint contraction = 0; contraction < contractions; contraction++) {
		scalar_type t0 = expf(-(factor_a_sh[contraction] * dist)) * factor_c_sh[contraction];
		t += t0;
  ...
  for (uint i = 0; i < functions.w; i += FUNCTIONS_BLOCK_SIZE) {
    if (i + threadIdx.x < functions.w) {
      ...
      for (uint contraction = 0; contraction < contractions_sh[threadIdx.x]; contraction++) {
        vec_type<scalar_type,2> factor_ac_local = factor_ac[COALESCED_DIMENSION(functions.w) * contraction + (i + threadIdx.x)];
