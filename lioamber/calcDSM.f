...
!!        scratch=t4*pis/RMM(M15+k-1)            !!!
        scratch=t4*pis                         !!!
        CALL assignDSM(i,j,Nuc(i),l2,scratch)  !!!

        ff(Nuc(j),l2)=ff(Nuc(j),l2)+t5*spi
!!        scratch=t5*spi/RMM(M15+k-1)            !!!
        scratch=t5*spi                         !!!
        CALL assignDSM(i,j,Nuc(j),l2,scratch)  !!!
...
!!        scratch=-ccoef*ss                       !!!
        scratch=-te*ss                         !!!
        CALL assignDSM(ii,j,Nuc(i),l2,scratch)  !!!
...
!!        scratch=t4*ds/RMM(M15+k-1)              !!!
        scratch=t4*ds                          !!!
        CALL assignDSM(ii,j,Nuc(i),l2,scratch)  !!!

        ff(Nuc(j),l2)=ff(Nuc(j),l2)+t5*pp
!!        scratch=t5*pp/RMM(M15+k-1)              !!!
        scratch=t5*pp                          !!!
        CALL assignDSM(ii,j,Nuc(j),l2,scratch)  !!!
...
!!        scratch=-ccoef*spj                       !!!
        scratch=-te*spj                         !!!
        CALL assignDSM(ii,jj,Nuc(i),l3,scratch)  !!!
       endif
c
       if (l2.eq.l3) then
        dp=dp+t11
...
!!        scratch=-ccoef*pis                       !!!
        scratch=-te*pis                         !!!
        CALL assignDSM(ii,jj,Nuc(j),l3,scratch)  !!!
       endif
c
       pd=dp+tx*pp
c
        ff(Nuc(i),l3)=ff(Nuc(i),l3)+t4*dp
!!        scratch=t4*dp/RMM(M15+k-1)               !!!
        scratch=t4*dp                           !!!
        CALL assignDSM(ii,jj,Nuc(i),l3,scratch)  !!!

        ff(Nuc(j),l3)=ff(Nuc(j),l3)+t5*pd
!!        scratch=t5*pd/RMM(M15+k-1)               !!!
        scratch=t5*pd                           !!!
        CALL assignDSM(ii,jj,Nuc(j),l3,scratch)  !!!
...
!!        scratch=-cc*pjs                         !!!
        scratch=-te*pjs                        !!!
        CALL assignDSM(ii,j,Nuc(i),l3,scratch)  !!!
        dp=dp+t11
       endif
c
       if (l2.eq.l3) then
...
!!        scratch=t4*fs/RMM(M15+k-1)              !!!
        scratch=t4*fs                          !!!
        CALL assignDSM(ii,j,Nuc(i),l3,scratch)  !!!

        ff(Nuc(j),l3)=ff(Nuc(j),l3)+t5*dp
!!        scratch=t5*dp/RMM(M15+k-1)              !!!
        scratch=t5*dp                          !!!
        CALL assignDSM(ii,j,Nuc(j),l3,scratch)  !!!
...
!!        scratch=-cc*pjpk                         !!!
        scratch=-te*pjpk                        !!!
        CALL assignDSM(ii,jj,Nuc(i),l4,scratch)  !!!
       endif
c
       if (l2.eq.l4) then
        dsd=dsd+t14
...
!!        scratch=-cc*dijs                         !!!
        scratch=-te*dijs                        !!!
        CALL assignDSM(ii,jj,Nuc(j),l4,scratch)  !!!
       endif
c
       fsp=dsd-tx*dijpk
c
        ff(Nuc(i),l4)=ff(Nuc(i),l4)+t4*fsp
!!        scratch=t4*fsp/RMM(M15+k-1)              !!!
        scratch=t4*fsp                          !!!
        CALL assignDSM(ii,jj,Nuc(i),l4,scratch)  !!!

        ff(Nuc(j),l4)=ff(Nuc(j),l4)+t5*dsd
!!        scratch=t5*dsd/RMM(M15+k-1)              !!!
        scratch=t5*dsd                          !!!
        CALL assignDSM(ii,jj,Nuc(j),l4,scratch)  !!!
...
!!         scratch=-cc*pjdkl                        !!!
         scratch=-te*pjdkl                       !!!
         CALL assignDSM(ii,jj,Nuc(i),l5,scratch)  !!!
        endif
c
        if (l2.eq.l5) then
         fd=fd+t11
...
!!         scratch=-cc*dijpl                        !!!
         scratch=-te*dijpl                       !!!
         CALL assignDSM(ii,jj,Nuc(j),l5,scratch)  !!!
        endif
c
        if (l4.eq.l5) then
         fd=fd+t13
...
!!        scratch=t4*fd/RMM(M15+k-1)               !!!
        scratch=t4*fd                           !!!
        CALL assignDSM(ii,jj,Nuc(i),l5,scratch)  !!!

        ff(Nuc(j),l5)=ff(Nuc(j),l5)+t5*df
!!        scratch=t5*df/RMM(M15+k-1)               !!!
        scratch=t5*df                           !!!
        CALL assignDSM(ii,jj,Nuc(j),l5,scratch)  !!!
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
       SUBROUTINE assignDSM(ii,jj,kk,dir,val)
!----------------------------------------------------------!
       USE garcha_mod
       INTEGER,INTENT(IN) :: ii,jj,kk,dir
       REAL*8,INTENT(IN)  :: val

       IF (dir.EQ.1) THEN
         DSX(ii,jj,kk)=DSX(ii,jj,kk)+val
         IF (ii.NE.jj) DSX(jj,ii,kk)=DSX(jj,ii,kk)+val
       ENDIF

       IF (dir.EQ.2) THEN
         DSY(ii,jj,kk)=DSY(ii,jj,kk)+val
         IF (ii.NE.jj) DSY(jj,ii,kk)=DSY(jj,ii,kk)+val
       ENDIF

       IF (dir.EQ.3) THEN
         DSZ(ii,jj,kk)=DSZ(ii,jj,kk)+val
