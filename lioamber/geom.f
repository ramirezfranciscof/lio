...
c       write(*,*) 'cosas',nt3,ntatom
      ...
      allocate(f(ntatom,3),f1(ntatom,3),f2(ntatom,3),dxyz(3,ntatom))
      allocate(xH(nt3,nt3),xU(nt3,3),xUU(nt3,3))
...
c        open(unit=69,file='opt.xyz')
c
      do 10 i=1,ntom
c      write(2,500) Iz(i),Pm(i),r(i,1),r(i,2),r(i,3)
       write(22,500) Iz(i),r(i,1),r(i,2),r(i,3)
  10  continue
c
c
...
c
        call dip(ux1,uy1,uz1)
        call dft_get_qm_forces(dxyz)

       do inan=1,natom
       f1(inan,1)=dxyz(1,inan)
       f1(inan,2)=dxyz(2,inan)
       f1(inan,3)=dxyz(3,inan)
       enddo

c        call int1G(f1)
c   
c
c        write(*,*) 'antes de int3G'
c        call int3G(f1,.false.)
c        write(*,*) 'despues de int3G'

c continuum solvent case
      if (field) then
        g1=g0
c
c        call dipg(ux1,uy1,uz1,f1)
      endif
c----------------------------
c classical solvent case ----
        if (sol) then
c         call mmsolG(natom,Nsol,natsol,Iz,pc,r,Em,Rm,f1)
c         call intsolG(f1)
        endif
c
...
c
        call dip(ux2,uy2,uz2)
       call dft_get_qm_forces(dxyz)

       do inan=1,natom
       f2(inan,1)=dxyz(1,inan)
       f2(inan,2)=dxyz(2,inan)
...
c continuum solvent case ----
      if (field) then
        g1=g0
c        call dipg(ux2,uy2,uz2,f2)
      endif
c
c----------------------------
c classical solvent case ----
        if (sol) then
c         call mmsolG(natom,Nsol,natsol,Iz,pc,r,Em,Rm,f2)
c         call intsolG(f2)
        endif
c---------------------------
c        call intSG(f2)
