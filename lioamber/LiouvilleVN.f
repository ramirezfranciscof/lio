!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
       SUBROUTINE LiouvilleVN(E,dipxyz)
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
!
!
!
...
!--------------------------------------------------------------------!
!      FFR VARS:
       INTEGER :: ErrID,iii,jjj,kkk,ndir
       LOGICAL :: docholesky,dolowdin
       REAL*8  :: fscratch
       REAL*8,ALLOCATABLE :: 
     >   fockaux(:,:),rhoaux(:,:),SMaux(:,:),
     >   rmmaux(:),QM(:,:),rhoaux2(:,:),rhoaux3(:,:),
     >   eigvec(:,:),eigval(:),Xfran(:,:)
!--------------------------------------------------------------------!
...
!----------------------------------------------------------!
! VARIABLE ALLOCATION - FFR
!
       ALLOCATE(DSX(M,M,natom),DSY(M,M,natom),DSZ(M,M,natom))
       ALLOCATE(rhoaux(M,M),fockaux(M,M),SMaux(M,M))
       ALLOCATE(rhoaux2(M,M),rhoaux3(M,M))
       ALLOCATE(rmmaux(MM),QM(M,M))
       ALLOCATE(eigvec(M,M),eigval(M))
!
!--------------------------------------------------------------------!
...
!----------------------------------------------------------!
       ...
       docholesky=.TRUE.
       dolowdin=.FALSE.
!       IF (.FALSE.) WRITE(*,*) 'Cholesky is: ',docholesky
...
!----------------------------------------------------------!
       ...
       IF (.FALSE.) THEN
         WRITE(6,*) 'Fockaux:'
         WRITE(6,520) fockaux(1,:)
         WRITE(6,520) fockaux(2,:)
         WRITE(6,520) fockaux(3,:)
...
!----------------------------------------------------------!
         ...
         IF (.FALSE.) THEN
           ...
           WRITE(6,520) rhoaux(2,:)
           WRITE(6,520) rhoaux(3,:)
           WRITE(6,520) rhoaux(4,:)
           WRITE(6,*) '---------------------'
         ENDIF
...
!
       ...
       DO i=j,M
         ...
         DO k=1,NCO
           RMM(M15+kk-1)=RMM(M15+kk-1)-
     >     RMM(M13+k-1)*ff*X(i,M2+k)*X(j,M2+k)
         ENDDO
