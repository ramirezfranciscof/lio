...
c            X(jjj,M2a+iii)=Xtrans(iii,jjj)
c        ENDDO;ENDDO
...
! Start use of RMM(M5) as Fock matrix (F')
...
c
       write(85,*)
       write(85,*) 'UNPAIRED SPIN MULLIKEN POPULATION ANALYSIS'
       write(85,770)
c
       do n=1,natom
         write(85,760) n,Iz(n),q(n)
...
c outputs final  MO ---------------------
      ...
      do l=1,M
        do n=1,NCOa
          kk=M18+(l-1)+M*(n-1)
          X(indexii(l),M+n)=RMM(kk)
        enddo
      enddo
c
      do l=1,M
        write(88,400) (X(l,M+n),n=1,NCOa)
      enddo
c
c-------------------------------------------------
c
