...
c Dario Estrin
c-------------------------------------------------------------------
      subroutine calcDSV(ff)
        use garcha_mod
c
      implicit real*8 (a-h,o-z)
      dimension ff(M*(M+1)/2,natom,3)
c
      dimension Q(3)
...
c
      do 205 l2=1,3
        ...
        spi=pis+ tx*ss
c
        ff(k,Nuc(i),l2)=ff(k,Nuc(i),l2)+t4*pis
        ff(k,Nuc(j),l2)=ff(k,Nuc(j),l2)+t5*spi
 205  continue
c
 200  continue
c
...
c
       if (l1.eq.l2) then
        ff(k,Nuc(i),l2)=ff(k,Nuc(i),l2)-te*ss
        ds=ds+t10
        pp=pp+t10
       endif
c
c
        ff(k,Nuc(i),l2)=ff(k,Nuc(i),l2)+t4*ds
        ff(k,Nuc(j),l2)=ff(k,Nuc(j),l2)+t5*pp
c
 307  continue
 305  continue
 300  continue
...
c
       if (l1.eq.l3) then
        dp=dp+t13
        ff(k,Nuc(i),l3)=ff(k,Nuc(i),l3)-te*spj
       endif
c
       if (l2.eq.l3) then
        dp=dp+t11
        ff(k,Nuc(j),l3)=ff(k,Nuc(j),l3)-te*pis
       endif
c
       pd=dp+tx*pp
c
        ff(k,Nuc(i),l3)=ff(k,Nuc(i),l3)+t4*dp
        ff(k,Nuc(j),l3)=ff(k,Nuc(j),l3)+t5*pd
c
 405  continue
c
c
...
c
       if (l1.eq.l3) then
        ff(k,Nuc(i),l3)=ff(k,Nuc(i),l3)-te*pjs
        dp=dp+t11
       endif
c
       if (l2.eq.l3) then
        ff(k,Nuc(i),l3)=ff(k,Nuc(i),l3)-te*pis
        dp=dp+t12
       endif
c
       fs=dp-tx*dijs
c
        ff(k,Nuc(i),l3)=ff(k,Nuc(i),l3)+t4*fs
        ff(k,Nuc(j),l3)=ff(k,Nuc(j),l3)+t5*dp
c
 505  continue
c
 500  continue
...
c
       if (l1.eq.l4) then
        dsd=dsd+t15
        ff(k,Nuc(i),l4)=ff(k,Nuc(i),l4)-te*pjpk
       endif
c
       if (l2.eq.l4) then
        dsd=dsd+t14
       ff(k,Nuc(i),l4)=ff(k,Nuc(i),l4)-te*pipk
       endif
c
       if (l3.eq.l4) then
        dsd=dsd+t13
       ff(k,Nuc(j),l4)=ff(k,Nuc(j),l4)-te*dijs
       endif
c
       fsp=dsd-tx*dijpk
c
        ff(k,Nuc(i),l4)=ff(k,Nuc(i),l4)+t4*fsp
        ff(k,Nuc(j),l4)=ff(k,Nuc(j),l4)+t5*dsd
c
 605  continue
c
 600  continue
...
c
        if (l1.eq.l5) then
        fd=fd+t10
        ff(k,Nuc(i),l5)=ff(k,Nuc(i),l5)-te*pjdkl
        endif
c
        if (l2.eq.l5) then
        fd=fd+t11
        ff(k,Nuc(i),l5)=ff(k,Nuc(i),l5)-te*pidkl
        endif
c
        if (l3.eq.l5) then
        fd=fd+t12
        ff(k,Nuc(j),l5)=ff(k,Nuc(j),l5)-te*dijpl
        endif
c
        if (l4.eq.l5) then
        fd=fd+t13
        ff(k,Nuc(j),l5)=ff(k,Nuc(j),l5)-te*dijpk
        endif
c
        df=fd+tx*ovlap
c
        ff(k,Nuc(i),l5)=ff(k,Nuc(i),l5)+t4*fd
        ff(k,Nuc(j),l5)=ff(k,Nuc(j),l5)+t5*df
c
 705  continue
c
 700  continue
